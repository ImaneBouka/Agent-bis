        ...
        if("scripts" in p_executionMessage && "run" in p_executionMessage.scripts)
        {
            ...
            if (process.platform === 'win32') {
                if (!global.sharedDrive) {
                    runScript = 'set ETAP_SHARED_DRIVE=%TEMP%' + os.EOL;
                    if (p_executionMessage.directory) {
                        runScript += 'set ETAP_EXECUTION_DIR=' + path.join('%TEMP%', p_executionMessage.directory.replace('/', '\\')) + os.EOL;
                        runScript += 'MKDIR %ETAP_EXECUTION_DIR%' + os.EOL;
    ...
    executeWithPreparatoryStep : function(functionToExecute, payload) {
        ...
        if (process.platform === 'win32') Executor.execute(l_preparatoryStepCommand)
            .then(function onSuccess() {
                logger.debug("Ok for preparatory steps.");
