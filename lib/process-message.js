...
var doProcessMessage = function (payload) {
    var msgObject = payload.content;
    var task = payload.task.toLowerCase().trim();
    ...
    switch (task) {
        case 'system':
            if (msgObject.executionID && msgObject.stopCommand) {
                if (m_executionID == msgObject.executionID) {
                    ...
                    console.log("onCancelMessage method called!!");
        ...
        case 'systemlowpriority':
            ...
            break;
        case 'jobs':
            var executionID = payload.content.executionID;
            if (!m_busy) {
                m_busy = true;
                m_executionID = executionID;
                m_currentTask = task;
                ...
                if (action && (action == "listen")) {
                    ...
                    m_executionHandler.onExecutionMessage(executionID)
                        .then(function allOK(data) {
                            ...
                            logger.debug('[e]execution on queue %s is done, perform acknowledge on message:[%s]', executionID, msgObject);
                            //if (m_hasPending > 0) {
                            doPendingMessage();
                            //}
                    ...
                    logger.debug('[e]executing job %s', msgObject);
            ...
            break;
        default:
            logger.debug('[a]perform ack on unknown routing key:%s msg:%s', '', payload.content);
            m_channel.ack(payload);
            //if (m_hasPending > 0) {
            doPendingMessage();
        //}
...
function initHandler(p_channel, p_connection) {
    ...
    if (!m_systemHandler) {
        ...
        m_systemHandler = new SystemHandler(g_broker, executor, m_channel, logger);
    } else {
    ...
    if (!m_executionHandler) {
        ...
        m_executionHandler = new ExecutionHandler(m_connection, g_broker, executor, m_channel, null, logger);
    } else {
...
//exports.getDeliveryTag = getDeliveryTag;